...
from get_data import get_world_data_from_csv, divide
from graph_plots import plot_bar_chart, plot_line_chart, display_kpi, display_world_map


def world_profit_analytics():

    st.title("International Profit Analytics")
    sales = get_world_data_from_csv()
    countries = pd.read_csv("countries.csv")

    sales_countries = pd.merge(sales,countries, on='Country')
    ...
    with segment_cell:
        customer_segment = selection_box(sales_countries, "Segment")

    sales_by_segment = sales_countries.query("Segment==@customer_segment")
    ...
    with country_cell:
        country = selection_box(sales_by_region, "Country")
    ...
    sales_queried = sales_countries.query(
        "Segment==@customer_segment & Region==@region & Country==@country"\
    ...
    with KPI_1:
        total_profit = int(sales_queried["Profit"].sum())
        display_kpi("Total Profit", "{:,}".format(total_profit),"$")

    with KPI_2:
        average_profit_per_transaction = round(sales_queried["Profit"].mean(),2)
        display_kpi("Average Profit per Transaction", "{:,}".format(average_profit_per_transaction), "$")

    with KPI_3:
        total_profit = int(sales_queried["Profit"].sum())
        months_number = len(sales_queried["Month_Year"].unique())
        average_profit_per_month = round(divide(total_profit, months_number),2)
        display_kpi("Average Profit per Month", "{:,}".format(average_profit_per_month), "$")
    ...
    with st.container():
        ...
        map = display_world_map(sales_queried, "<b>Profit per Country($)</b>", map_lat, map_long, map_color, np.absolute((sales_queried.index)))
